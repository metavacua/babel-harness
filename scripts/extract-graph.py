...
# ---------------------------------------------------------------------------
# Phase 3: Vindexfile generation (with optional remote graph inclusion)
# ---------------------------------------------------------------------------

def _vindexfile_header(vindex_base: str,
                       remote_refs: list[str] | None = None) -> str:
    """Build Vindexfile header, optionally listing remote graph sources."""
    remote_lines = ""
    if remote_refs:
        remote_lines = "\n" + "\n".join(f"# FROM github://{r}" for r in remote_refs)
    regen_remote = ""
    if remote_refs:
        regen_remote = " " + " ".join(f"--remote {r}" for r in remote_refs)
    return (
        f"# Re-generate: LARQL_VINDEX_BASE=/path/to/larql-vindexes"
        f" python3 scripts/extract-graph.py{regen_remote}\n"
        f"FROM {vindex_base}/smollm2-360m.vindex{remote_lines}\n"
        "\n"
...
def _parse_lql_inserts(lql_text: str) -> list[tuple]:
    """Parse (from, relation, to, 1.0) tuples from LQL INSERT statements."""
    edges = []
    for m in re.finditer(
            r'^INSERT\s+"([^"]+)",\s*"([^"]+)",\s*"([^"]+)"', lql_text, re.MULTILINE):
        edges.append((m.group(1), m.group(2), m.group(3), 1.0))
    return edges


def _fetch_remote_graph(owner_repo_ref: str, script_path: str) -> list[tuple]:
    """
    Fetch INSERT triples from a remote GitHub repo via github_graph.py.

    Satisfies sub-condition 3: GitHub repos as hosted remote vindexes.
    The fetched triples are merged into the local Vindexfile so the remote
    repo's entity graph is grafted into the larql vindex alongside babel-harness.
    Non-blocking: returns [] on any failure, with a warning to stderr.
    """
    parts = owner_repo_ref.split("@", 1)
    repo_part = parts[0]
    ref = parts[1] if len(parts) > 1 else "main"
    try:
        result = subprocess.run(
            [sys.executable, script_path,
             "--repo", repo_part, "--ref", ref, "--output", "lql"],
            capture_output=True, text=True, timeout=120
        )
        if result.returncode != 0:
            err = (result.stderr or result.stdout).strip()[:200]
            print(f"[graph] Warning: remote graph {owner_repo_ref} failed: {err}",
                  file=sys.stderr)
            return []
        return _parse_lql_inserts(result.stdout)
    except subprocess.TimeoutExpired:
        print(f"[graph] Warning: timeout fetching remote graph {owner_repo_ref}",
              file=sys.stderr)
        return []
    except Exception as e:
        print(f"[graph] Warning: could not fetch remote graph {owner_repo_ref}: {e}",
              file=sys.stderr)
        return []
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract babel-harness as a LARQL knowledge graph and Vindexfile.")
    parser.add_argument(
        "--remote", metavar="OWNER/REPO[@REF]", action="append", default=[],
        help="Merge a remote GitHub repo graph into the Vindexfile (repeatable). "
             "Example: --remote chrishayuk/larql@main")
    parser.add_argument(
        "--remote-script",
        default=str(REPO / "scripts" / "github_graph.py"),
        help="Path to github_graph.py (default: scripts/github_graph.py)")
    args = parser.parse_args()

    vindex_base = os.environ.get("LARQL_VINDEX_BASE", "")
    if not vindex_base:
        print("error: LARQL_VINDEX_BASE env var is required\n"
              "  e.g. LARQL_VINDEX_BASE=/path/to/larql-vindexes "
    ...
    # Phase 3: merge remote GitHub repo graphs into the Vindexfile
    # Each --remote spec fetches the repo's INSERT triples via github_graph.py
    # and grafts them into the knowledge graph before ternary encoding.
    if args.remote:
        existing_entities: set[str] = set(entities)
        for ref_spec in args.remote:
            print(f"[graph] Fetching remote graph: github://{ref_spec}", file=sys.stderr)
            remote_edges = _fetch_remote_graph(ref_spec, args.remote_script)
            if remote_edges:
                edges.extend(remote_edges)
                for frm, _rel, to, _conf in remote_edges:
                    for eid in (frm, to):
                        if eid not in existing_entities:
                            existing_entities.add(eid)
                            entities.append(eid)
                print(f"[graph]   merged {len(remote_edges)} triples from {ref_spec}",
                      file=sys.stderr)
    ...
    write_vindexfile(trits, entities, edges, repo / 'Vindexfile',
                     vindex_base, remote_refs=args.remote or None)
